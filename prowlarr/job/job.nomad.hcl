  ...
  group "prowlarr" {
    ...
    service {
      ...
      tags = [
        ...
        "traefik.http.routers.prowlarr.rule=Host(`prowlarr.ednz.fr`)",
        "traefik.http.routers.prowlarr.tls.certresolver=cloudflare",
        "traefik.http.routers.prowlarr.middlewares=internal-acl@consulcatalog,authentik@consulcatalog"
      ]
      connect {
        sidecar_service {
          proxy {
    ...
    task "borg-sidecar" {
      ...
      volume_mount {
        volume      = "prowlarr-data"
        destination = "/backup-prowlarr-data"
      }
      resources {
        cpu        = 200
        memory     = 20
...
variable "prowlarr_prowlarr_env" {
  type = string
}
