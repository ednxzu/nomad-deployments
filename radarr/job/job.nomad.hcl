  ...
  group "radarr" {
    ...
    service {
      ...
      tags = [
        ...
        "traefik.http.routers.radarr.rule=Host(`radarr.ednz.fr`)",
        "traefik.http.routers.radarr.tls.certresolver=cloudflare",
        "traefik.http.routers.radarr.middlewares=internal-acl@consulcatalog,authentik@consulcatalog"
      ]
      connect {
        sidecar_service {
          proxy {
    ...
    task "borg-sidecar" {
      ...
      volume_mount {
        volume      = "radarr-data"
        destination = "/backup-radarr-data"
      }
      resources {
        cpu        = 200
        memory     = 20
