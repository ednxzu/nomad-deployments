locals {
  ...
  jobs = {
    overseerr = "${path.module}/job/job.nomad.hcl"
  }

  jobs_variables = {
    overseerr = {
      overseerr_overseerr_env       = base64encode(file("${path.module}/job/config/overseerr/overseerr.env"))
  ...
  consul_intentions = {
    traefik-to-overseerr = {
      source_name      = "traefik"
      destination_name = "overseerr"
      action           = "allow"
    }
    overseerr-to-radarr = {
      source_name      = "overseerr"
