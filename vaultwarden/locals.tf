locals {
  ...
  jobs_variables = {
    vaultwarden = {
      vaultwarden_vaultwarden_env   = base64encode(file("${path.module}/job/config/vaultwarden/vaultwarden.env"))
  ...
  consul_intentions = {
    traefik-to-vaultwarden = {
      source_name      = "traefik"
      destination_name = "vaultwarden"
      action           = "allow"
    }
  }
}
